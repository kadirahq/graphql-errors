...
// Use `UserError` to send error messages to users.
export const InternalError = new Error('internal error');


// If a `UserError` is thrown when resolving the field
...
export function processField(field) {
  ...
  field.resolve = async function (...args) {
    try {
      ...
      // TODO
      // `e instanceof UserError` didn't work
      // using a temporary field until it's fixed.
      if (e._isUserError) {
        throw e;
      }
